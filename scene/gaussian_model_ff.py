...
class FeatureGaussianModel:
    ...
    def load_from_4dgs(self, model_params):
        # (model_params, first_iter) = torch.load(ckpt_path)

        # assume gaussian_dim == 4
        self._xyz = model_params[1]
        self._scaling = model_params[4]
        self._rotation = model_params[5]
        self._opacity = model_params[6]
        self._t = model_params[13]
        self._scaling_t = model_params[14]
        self._rotation_r = model_params[15]

        features = torch.zeros((self._xyz.shape[0], self.feature_dim)).float().cuda()
        self._point_features = nn.Parameter(features.contiguous().requires_grad_(True))
    ...
    def densification_postfix(self, new_xyz, new_point_features, new_opacities, new_scaling, new_rotation, new_t, new_scaling_t, new_rotation_r):
        d = {"xyz": new_xyz,
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, grads_t, grad_t_threshold, N=2):
        ...
        self.densification_postfix(new_xyz, new_point_features, new_opacity, new_scaling, new_rotation, new_t, new_scaling_t, new_rotation_r)
    ...
    def densify_and_clone(self, grads, grad_threshold, scene_extent, grads_t, grad_t_threshold):
        ...
        self.densification_postfix(new_xyz, new_point_features, new_opacities, new_scaling, new_rotation, new_t, new_scaling_t, new_rotation_r)
