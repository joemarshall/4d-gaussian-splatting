...
class GaussianModel:
    ...
    def _make_save_or_restore_calls(self, fn_to_call, argument_in):
        ...
        if argument_in is not None:
            for var_name in vars_to_save:
                if var_name in argument_in:
                    all_vars[var_name] = fn_to_call(
                        self, var_name, argument_in[var_name]
                    )
                else:
                    print(
                        "Missing variable {} in input data for save/restore, skipping.".format(
                            var_name
                        )
                    )
        else:
        ...
        for densifier in self.densifiers:
            ...
            if (
                argument_in is not None
                and densifier_name in argument_in["densifier_vars"]
            ):
            ...
            for var_name in densifier_vars:
                if argument_in is not None and var_name in densifier_in_args:
                    densifier_out_vars[var_name] = fn_to_call(
                        densifier, var_name, densifier_in_args[var_name]
                    )
                else:
    ...
    def restore(self, model_args, training_args):
        ...
        if gaussian_dim != self.gaussian_dim:
            raise ValueError(
                f"Gaussian dimension mismatch in load: expected {self.gaussian_dim}, got {gaussian_dim}"
            )
        ...
        optimizer_params = [
            x
            for x in restore_return_values.values()
            ...
            if group["name"] == name:
                ...
                if stored_state is not None:
                    ...
                    group["params"][0] = nn.Parameter(
                        (group["params"][0][mask].detach().requires_grad_(True))
                    )
                    self.optimizer.state[group["params"][0]] = stored_state
            ...
            if group["name"] == name:
                ...
                if stored_state is not None:
                    ...
                    stored_state["exp_avg"] = torch.cat(
                        (stored_state["exp_avg"].detach(), torch.zeros_like(tensor)),
                        dim=0,
                    ).requires_grad_(True)
                    stored_state["exp_avg_sq"] = torch.cat(
                        (stored_state["exp_avg_sq"].detach(), torch.zeros_like(tensor)),
                        dim=0,
                    ).requires_grad_(True)
                    ...
                    group["params"][0] = nn.Parameter(
                        torch.cat(
                            (group["params"][0].detach(), tensor.detach()), dim=0
                        ).requires_grad_(True)
                    )
                    self.optimizer.state[group["params"][0]] = stored_state
                    ...
                    group["params"][0] = nn.Parameter(
                        torch.cat(
                            (group["params"][0].detach(), tensor), dim=0
                        ).requires_grad_(True)
                    )
                    return group["params"][0]
    ...
    def add_densification_stats(
        ...
        *,
        ...
        for densifier in self.densifiers:
            densifier.add_densification_stats(
                self,
                iteration=iteration,
                viewspace_point_tensor=viewspace_point_tensor,
                update_filter=update_filter,
                radii=radii,
                avg_t_grad=avg_t_grad,
            )

    def add_densification_stats_grad(
        self, *, iteration, viewspace_point_grad, update_filter, radii, avg_t_grad=None
    ):
        for densifier in self.densifiers:
            densifier.add_densification_stats_grad(
                gaussians=self,
                iteration=iteration,
                viewspace_point_grad=viewspace_point_grad,
                update_filter=update_filter,
                radii=radii,
                avg_t_gradient=avg_t_grad,
            )
    ...
    def run_densifiers(self, iteration, scene, radii, pipe, bg):
        for densifier in self.densifiers:
            ...
            if options is not None:
                densifier.densify_and_prune(
                    iteration, scene, self, radii, pipe, bg, options=options
                )
