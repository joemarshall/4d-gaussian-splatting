...
def training(dataset, opt, pipe, iteration, saving_iterations, checkpoint, checkpoint_feature, debug_from,
             gaussian_dim=4, time_duration=[0.0, 3.0], rot_4d=True, force_sh_3d=False):
    ...
    assert checkpoint is not None, 'feature training needs gaussian ckpt'
    if checkpoint:
    ...
    scene = Scene(
        ...
        load_iteration=iteration, shuffle=False, target='contrastive_feature', mode='train', sample_rate=sample_rate)

    if checkpoint_feature:
        print(f"load feature gaussian from {checkpoint_feature}")
        (model_params, first_iter) = torch.load(checkpoint_feature)
        feature_gaussians.restore(model_params, opt)
    elif checkpoint:
        print(f"load feature gaussian from {checkpoint}")
        (model_params, first_iter) = torch.load(checkpoint)
        feature_gaussians.load_from_4dgs(model_params)
    else:
        print(f"init feature gaussian...")
        feature_gaussians.create_from_pcd(scene.scene_info.point_cloud, scene.cameras_extent)
    ...
    print(f"upper_bound_scale: {upper_bound_scale}")
    ...
    while iteration < opt.iterations + 1:
        for batch_data in training_dataloader:  # batch size == 1
            iteration += 1
            iter_start.record()
            ...
            gt_image, viewpoint_cam, features, original_masks, mask_scales = batch_data[0]
            viewpoint_cam = viewpoint_cam.cuda()

            with torch.no_grad():
                # N_mask, H, W
                # sam_masks = viewpoint_cam.original_masks.cuda().float()
                sam_masks = original_masks.cuda().float()
            ...
            rendered_features = torch.nn.functional.interpolate(rendered_features.unsqueeze(0), original_masks.shape[-2:], mode='bilinear').squeeze(0)
    ...
    torch.save(scale_gate.state_dict(), os.path.join(scene.model_path, f"scale_gate{iteration}.pt"))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--start_checkpoint_feature", type=str, default=None)
    ...
    parser.add_argument("--time_duration", nargs=2, type=float, default=[0.0, 0.5])
    ...
    training(lp.extract(args), op.extract(args), pp.extract(args),
             args.iteration, args.save_iterations,
             args.start_checkpoint, args.start_checkpoint_feature,
             args.debug_from,
