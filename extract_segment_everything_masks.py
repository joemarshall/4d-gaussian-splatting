...
if __name__ == '__main__':
    ...
    parser.add_argument("--sam_checkpoint_path", default='./third_party/segment-anything/sam_ckpt/sam_vit_h_4b8939.pth',
                        type=str)
    ...
    for path in tqdm(sorted(os.listdir(IMAGE_DIR))):
        ...
        if postfix != 'png' and postfix != 'jpg':
            continue
        ...
        for m in masks:
            ...
            if args.downsample_type == 'mask':
                m_score = torch.nn.functional.interpolate(m_score.unsqueeze(0).unsqueeze(0), size=(
                img.shape[0] // args.downsample, img.shape[1] // args.downsample), mode='bilinear',
