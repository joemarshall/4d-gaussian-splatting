...
def render_stochastic(
    ...
    """Stochastic Gaussian rendering – pure PyTorch, no C extensions.

    Drop-in replacement for ``render()``.  Uses the stochastic rendering
    algorithm from https://arxiv.org/html/2503.24366v1:

    For each pixel, ``num_samples`` Gaussians are drawn proportional to their
    alpha × transmittance contribution and the pixel colour is estimated as:

        C_hat = Z * mean(c_sampled) + T_N * bg

    where Z = 1 − T_N is the accumulated alpha.

    Parameters
    ----------
    num_samples : int
        Number of Gaussian samples per pixel.
        Set to ``-1`` (or any non-positive value) for full deterministic
        alpha compositing (identical result to the CUDA renderer).
        Default: 4
    tile_size : int
        Tile side length in pixels used for spatial batching.
        Default: 16

    Returns
    -------
    dict with keys identical to ``render()``:
        render, viewspace_points, visibility_filter, radii, depth, alpha, flow
    """
    from .stochastic_rasterization import (
        GaussianRasterizationSettings as SGRSettings,
        StochasticGaussianRasterizer,
    )

    # Zero tensor: receives gradients of the 2-D projected positions
    screenspace_points = (
        torch.zeros_like(pc.get_xyz, dtype=pc.get_xyz.dtype,
                         requires_grad=True, device=pc.get_xyz.device) + 0
    )
    try:
        screenspace_points.retain_grad()
    except Exception:
    ...
    raster_settings = SGRSettings(
        image_height=int(viewpoint_camera.image_height),
        image_width=int(viewpoint_camera.image_width),
        tanfovx=tanfovx,
        tanfovy=tanfovy,
        bg=bg_color if not pipe.env_map_res else torch.zeros(3, device=bg_color.device),
        scale_modifier=scaling_modifier,
        viewmatrix=viewpoint_camera.world_view_transform,
        projmatrix=viewpoint_camera.full_proj_transform,
        sh_degree=pc.active_sh_degree,
    ...
    rasterizer = StochasticGaussianRasterizer(
        ...
        if pipe.convert_SHs_python:
            ...
            if pipe.compute_cov3D_python:
                dir_pp = (means3D - viewpoint_camera.camera_center.repeat(
                    pc.get_features.shape[0], 1)).detach()
            else:
                _, delta_mean = pc.get_current_covariance_and_mean_offset(
                    scaling_modifier, viewpoint_camera.timestamp)
                dir_pp = ((means3D + delta_mean)
            ...
            if pc.gaussian_dim == 3 or pc.force_sh_3d:
                ...
                dir_t = (pc.get_t - viewpoint_camera.timestamp).detach()
                sh2rgb = eval_shfs_4d(
                    pc.active_sh_degree, pc.active_sh_degree_t,
                    shs_view, dir_pp_normalized, dir_t,
    ...
    # Prefilter (same as in render())
    if pipe.compute_cov3D_python and pc.gaussian_dim == 4:
        mask = marginal_t[:, 0] > 0.05
        if means2D is not None:     means2D = means2D[mask]
        if means3D is not None:     means3D = means3D[mask]
        if ts is not None:          ts = ts[mask]
        if shs is not None:         shs = shs[mask]
        if colors_precomp is not None: colors_precomp = colors_precomp[mask]
        if opacity is not None:     opacity = opacity[mask]
        if scales is not None:      scales = scales[mask]
        if scales_t is not None:    scales_t = scales_t[mask]
        if rotations is not None:   rotations = rotations[mask]
        if rotations_r is not None: rotations_r = rotations_r[mask]
        if cov3D_precomp is not None: cov3D_precomp = cov3D_precomp[mask]
        if flow_2d is not None:     flow_2d = flow_2d[mask]

    rendered_image, radii, depth, alpha, flow, _covs_com = rasterizer(
        means3D=means3D,
        means2D=means2D,
        shs=shs,
        colors_precomp=colors_precomp,
    ...
    if pipe.env_map_res:
        ...
        R = 60
        rays_o, rays_d = viewpoint_camera.get_rays()
        delta = ((rays_o * rays_d).sum(-1)) ** 2 - (rays_d ** 2).sum(-1) * (
            (rays_o ** 2).sum(-1) - R ** 2)
        assert (delta > 0).all()
        t_inter = (-(rays_o * rays_d).sum(-1)
                   + torch.sqrt(delta) / (rays_d ** 2).sum(-1))
        xyz_inter = rays_o + rays_d * t_inter.unsqueeze(-1)
        tu = torch.atan2(xyz_inter[..., 1:2], xyz_inter[..., 0:1]) / (
            2 * torch.pi) + 0.5
        tv = torch.acos(xyz_inter[..., 2:3] / R) / torch.pi
        texcoord = torch.cat([tu, tv], dim=-1) * 2 - 1
        bg_color_from_envmap = F.grid_sample(
            pc.env_map[None], texcoord[None], align_corners=False)[0]
