...
if __name__ == '__main__':
    ...
    (model_params, first_iter) = torch.load(
        os.path.join(args.model_path, 'chkpnt30000.pth')
    )
    scene_gaussians.restore(model_params, None)

    scene = Scene(
        dataset, scene_gaussians, feature_gaussians,
        time_duration=time_duration,
        load_iteration=-1, feature_load_iteration=-1, shuffle=False, mode='eval', target='scene'
    )
    ...
    # images_masks = {}
    # for i, image_path in tqdm(enumerate(sorted(os.listdir(os.path.join(dataset.source_path, 'sam_masks'))))):
    #     # image = cv2.imread(os.path.join(os.path.join(dataset.source_path, 'images'), image_path.replace('pt', 'png')))
    #     # image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
    #     masks = torch.load(os.path.join(os.path.join(dataset.source_path, 'sam_masks'), image_path))
    #     # N_mask, C
    #
    ...
    for data in training_dataset:
        view_image, view, _, _, _  = data
        view = view.cuda()

        save_path = os.path.join(OUTPUT_DIR, view.image_name + '.pt')
        if os.path.exists(save_path):
            continue

        rendered_pkg = render(view, scene_gaussians, pipeline.extract(args), background)
        ...
        corresponding_masks = torch.load(
            os.path.join(dataset.source_path, 'sam_masks', f'{view.image_name}.pt')
        )
        ...
        # cx = depth.shape[1] / 2
        # cy = depth.shape[0] / 2
        # fx = cx / np.tan(view[0].FoVx / 2)
        # fy = cy / np.tan(view[0].FoVy / 2)
        cx = view.cx
        cy = view.cy
        fx = view.fl_x
        fy = view.fl_y
        ...
        eroded_masks = torch.conv2d(
            ...
            torch.full((3, 3), 1.0).view(1, 1, 3, 3).cuda(),
            padding=1,
        )
