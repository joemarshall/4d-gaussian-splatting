...
class StableDiffusion(nn.Module):
    ...
    @torch.no_grad()
    def get_text_embeds(self, prompts, negative_prompts, prompts_emb=None):
        if prompts_emb is None:
            pos_embeds = self.encode_text(prompts)  # [1, 77, 768]
        else:
            pos_embeds = prompts_emb[None, ...]
        neg_embeds = self.encode_text(negative_prompts)
        self.embeddings = torch.cat([neg_embeds, pos_embeds], dim=0).to(self.dtype)  # [2, 77, 768]
