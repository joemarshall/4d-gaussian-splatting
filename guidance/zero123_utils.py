...
class Zero123(nn.Module):
    def __init__(self, device, fp16=True, t_range=[0.02, 0.98], stable=False):
        super().__init__()
        ...
        model_key = "ashawkey/stable-zero123-diffusers" if stable else "ashawkey/zero123-xl-diffusers"
        ...
        self.use_stable_zero123 = stable
    ...
    def get_cam_embeddings(self, elevation, azimuth, radius, default_elevation=0):
        if self.use_stable_zero123:
            T = np.stack([np.deg2rad(elevation), np.sin(np.deg2rad(azimuth)), np.cos(np.deg2rad(azimuth)),
                          np.deg2rad([90 + default_elevation] * len(elevation))], axis=-1)
        else:
            # original zero123 camera embedding
            T = np.stack([np.deg2rad(elevation), np.sin(np.deg2rad(azimuth)), np.cos(np.deg2rad(azimuth)), radius],
                         axis=-1)
    ...
    def refine(self, pred_rgb, elevation, azimuth, radius,
               guidance_scale=5, steps=50, strength=0.8, default_elevation=0, embeddings=None
               ):
    ...
    def train_step(self, pred_rgb, elevation, azimuth, radius,
                   step_ratio=None, guidance_scale=5, as_latent=False, default_elevation=0, embeddings=None):
