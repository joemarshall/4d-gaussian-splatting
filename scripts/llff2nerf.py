...
if __name__ == '__main__':
    ...
    test_ids = np.array([i for i in all_ids if images[i].split("/")[1].split('_')[0] == 'cam00'])
    train_ids = np.array([i for i in all_ids if images[i].split("/")[1].split('_')[0] != 'cam00'])
    times_ids = [int(os.path.basename(images[i]).split('.')[0].split('_')[1]) for i in all_ids]
    poses_ids = [cam_name_map[os.path.basename(images[i]).split('.')[0].split('_')[0]] for i in all_ids]

    frames_train = []
    frames_test = []
    for i in train_ids:
        frames_train.append({
            'file_path': images[i].split('.')[-2],
            'transform_matrix': poses[poses_ids[i]].tolist(),
            'time': times_ids[i] / 30.
        })
    for i in test_ids:
        frames_test.append({
