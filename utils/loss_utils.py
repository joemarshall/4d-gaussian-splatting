...
def loss_ssim(
    render_package, gt_image, gt_depth, gaussians, args, iterations, max_iterations
):
    return 1.0 - ssim(render_package["render"], gt_image)


def loss_bistable_opacity(
    render_package, gt_image, gt_depth, gaussians, args, iterations, max_iterations
):
...
def loss_depth(
    ...
    depth_out_scaled = _rescale_depth(gt_depth,depth_out)
    return l1_loss(gt_depth, depth_out_scaled)
    ...
    # max_gt_depths = torch.nn.MaxPool2d(3, stride=1, padding=1)(
    #     gt_depth.unsqueeze(0).unsqueeze(0)
    # ).squeeze()
    # min_gt_depths = -torch.nn.MaxPool2d(3, stride=1, padding=1)(
    #     -gt_depth.unsqueeze(0).unsqueeze(0)
    # ).squeeze()
